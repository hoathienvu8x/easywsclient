...
namespace { // private module-only namespace
  ...
  class _RealWebSocket : public easywsclient::WebSocket {
    ...
    // +---------------------------------------------------------------+
      ...
      _RealWebSocket(socket_t sockfd, bool useMask) : sockfd(sockfd),
        readyState(OPEN) , useMask(useMask) , isRxBad(false) {}
      ...
      void poll(int timeout) { // timeout in milliseconds
        ...
        if (timeout != 0) {
          ...
          if (select(sockfd + 1, &rfds, &wfds, 0, timeout > 0 ? &tv : 0) <= 0) {
            terminate();
          }
        }
        while (true) {
      ...
      virtual void _dispatchBinary(BytesCallback_Imp & callable) {
        ...
        while (true) {
          ...
          if (rxbuf.size() < ws.header_size) {
            /* Need: ws.header_size - rxbuf.size() */
            return;
          }
          int i = 0;
          if (ws.N0 < 126) {
          ...
          if (
            ...
            receivedData.insert(
              receivedData.end(), rxbuf.begin()+ws.header_size,
            ...
            sendData(
              wsheader_type::PONG, data.size(), data.begin(), data.end()
            );
          ...
          rxbuf.erase(
            rxbuf.begin(), rxbuf.begin() + ws.header_size+(size_t)ws.N
          );
        }
      }
      ...
      void sendData(
        wsheader_type::opcode_type type, uint64_t message_size,
  ...
  easywsclient::WebSocket::pointer from_url(
    const std::string& url, bool useMask, const std::string& origin
  ) {
    char host[512];
    int port;
    char path[512];
    ...
    if (origin.size() >= 200) {
      fprintf(
        stderr, "ERROR: origin size limit exceeded: %s\n", origin.c_str()
      );
      return NULL;
    }
    if (sscanf(url.c_str(), "ws://%[^:/]:%d/%s", host, &port, path) == 3) {
      ...
      fprintf(
        stderr, "ERROR: Could not parse WebSocket url: %s\n", url.c_str()
      );
      return NULL;
    }
    //fprintf(
    //  stderr, "easywsclient: connecting: host=%s port=%d path=/%s\n",
    // host, port, path
    //);
    ...
    }
    {
      ...
      std::string websocket_key = base64_encode(key_nonce, 16);
      ...
      if (i == 1023) {
        fprintf(
          stderr, "ERROR: Got invalid status line connecting to: %s\n",
          url.c_str()
        );
        return NULL;
      }
      if (sscanf(line, "HTTP/1.1 %d", &status) != 1 || status != 101) {
        fprintf(
          stderr, "ERROR: Got bad status connecting to %s: %s",
          url.c_str(), line
        );
        return NULL;
      }
...
namespace easywsclient {
  ...
  WebSocket::pointer WebSocket::from_url(
    const std::string& url, const std::string& origin
  ) {
