...
namespace {
    ...
    class _DummyWebSocket : public wsclient::WebSocket {
        public:
            ...
            void _dispatch(wsclient::Callback_Imp & callable) { }
            void _dispatchBinary(wsclient::BytesCallback_Imp& callable) { }
    };
    class _RealWebSocket : public wsclient::WebSocket {
        ...
        virtual void _dispatch(wsclient::Callback_Imp & callable) {
            struct CallbackAdapter : public wsclient::BytesCallback_Imp {
                wsclient::Callback_Imp& callable;
                CallbackAdapter(wsclient::Callback_Imp& callable) : callable(callable) { }
        ...
        virtual void _dispatchBinary(wsclient::BytesCallback_Imp & callable) {
            if (isRxBad) {
                return;
            }
            while (true) {
    ...
    wsclient::WebSocket::pointer from_url(const std::string& ourl, bool useMask, const std::string& origin) {
        char host[512];
        int port;
        char path[512];
        std::string url = ourl;
